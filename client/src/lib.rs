...
impl Default for PlayerPhysics {
    fn default() -> Self {
        Self { velocity: Vec3::ZERO, on_ground: false, jump_cooldown: 0.0 }
    }
}
...
pub fn run_client() {
    ...
    App::new()
        .add_plugins(DefaultPlugins
            // 🔑 CHEMIN ASSETS: pointe vers la racine du workspace
            .set(AssetPlugin { file_path: "../assets".into(), ..Default::default() })
            .set(WindowPlugin {
                primary_window: Some(Window {
                    title: "🌿 Symbia HD".into(),
                    resolution: (1280., 720.).into(),
                    resizable: true,
                    // ✅ Démarrage SÉCURISÉ : curseur visible et libre
                    cursor: Cursor { 
...
fn setup_world(mut commands: Commands, mut materials: ResMut<Assets<StandardMaterial>>, 
               mut images: ResMut<Assets<Image>>, asset_server: Res<AssetServer>) {
    let atlas_handle = load_or_create_atlas_sync(&asset_server, &mut images);

    // 🎨 Filtrage : 
    // - Nearest : Garde l'aspect "Pixel Art" net (recommandé pour Minecraft-like)
    // - Linear : Rend les textures plus floues mais plus douces (plus réaliste)
    // J'ai mis Nearest pour garder le style net de tes blocs.
    ...
    let mat = materials.add(StandardMaterial {
        base_color: Color::rgb(1.2, 1.2, 1.2), base_color_texture: Some(atlas_handle),
        perceptual_roughness: 0.7, metallic: 0.0, reflectance: 0.1,
        alpha_mode: AlphaMode::Opaque, ..Default::default()
    });
    ...
    commands.spawn((Camera3dBundle {
        ...
        ..Default::default()
    }, PlayerCamera, ctrl, PlayerPhysics::default()));

    commands.spawn(DirectionalLightBundle {
        directional_light: DirectionalLight { illuminance: 8000.0, shadows_enabled: true, shadow_depth_bias: 0.02, ..Default::default() },
        transform: Transform::from_xyz(20.0, 40.0, 20.0).looking_at(Vec3::ZERO, Vec3::Y), ..Default::default()
    });
    commands.spawn(PointLightBundle {
        point_light: PointLight { color: Color::rgb(0.8, 0.85, 1.0), intensity: 200.0, radius: 100.0, ..Default::default() },
        transform: Transform::from_xyz(0.0, 50.0, 0.0), ..Default::default()
    });
...
// 🖱️ Système unifié de gestion du curseur
fn manage_cursor(
    ...
    if key_input.just_pressed(KeyCode::Escape) {
        *is_captured = !*is_captured;
        if *is_captured { println!("▶️ Repris"); } else { println!("⏸️ Pause (Clic gauche pour reprendre)"); }
    }
...
fn keyboard_with_physics(time: Res<Time>, mut q: Query<(&mut Transform, &mut CameraController, &mut PlayerPhysics), With<PlayerCamera>>, 
                         world: Res<ChunkWorld>, keys: Res<ButtonInput<KeyCode>>) {
    let (mut t, ctrl, mut p) = match q.get_single_mut() { Ok(x) => x, Err(_) => return };
    ...
    if inp.length_squared() > 0. { inp = inp.normalize(); p.velocity.x = inp.x*ctrl.speed; p.velocity.z = inp.y*ctrl.speed; }
    else { p.velocity.x *= 0.9; p.velocity.z *= 0.9; if p.velocity.x.abs()<0.1{p.velocity.x=0.;} if p.velocity.z.abs()<0.1{p.velocity.z=0.;} }
    p.jump_cooldown -= dt;
    if keys.just_pressed(KeyCode::Space) && p.on_ground && p.jump_cooldown <= 0. { p.velocity.y = JUMP_VELOCITY; p.on_ground = false; p.jump_cooldown = JUMP_COOLDOWN; }
    if !p.on_ground { p.velocity.y += GRAVITY*dt; if p.velocity.y < TERMINAL_VELOCITY { p.velocity.y = TERMINAL_VELOCITY; } }
    let mut pos = t.translation; let (hw, hb, ht) = (0.3, 0.85, 0.85);
    pos.x += p.velocity.x*dt; if check_aabb_collision(&world, Vec3::new(pos.x-hw,pos.y-hb,pos.z-hw), Vec3::new(pos.x+hw,pos.y+ht,pos.z+hw)) { pos.x -= p.velocity.x*dt; p.velocity.x=0.; }
    pos.z += p.velocity.z*dt; if check_aabb_collision(&world, Vec3::new(pos.x-hw,pos.y-hb,pos.z-hw), Vec3::new(pos.x+hw,pos.y+ht,pos.z+hw)) { pos.z -= p.velocity.z*dt; p.velocity.z=0.; }
    ...
    if check_aabb_collision(&world, Vec3::new(pos.x-hw,pos.y-hb,pos.z-hw), Vec3::new(pos.x+hw,pos.y+ht,pos.z+hw)) {
        if p.velocity.y < 0. { pos.y = (pos.y-hb).floor()+hb+1.; p.velocity.y=0.; p.on_ground=true; }
        else { pos.y = (pos.y+ht).floor()+ht-1.; p.velocity.y=0.; }
    } else { p.on_ground = false; }
    if !p.on_ground && p.velocity.y <= 0. { let fp = Vec3::new(pos.x, pos.y-hb-GROUND_CHECK_DIST, pos.z); if is_block_solid(&world, fp.x.floor() as i32, fp.y.floor() as i32, fp.z.floor() as i32) { p.on_ground = true; } }
    t.translation = pos;
}

fn handle_block_interaction(mut commands: Commands, mut meshes: ResMut<Assets<Mesh>>, mut world: ResMut<ChunkWorld>, 
                            query: Query<&Transform, With<PlayerCamera>>, mouse: Res<ButtonInput<bevy::input::mouse::MouseButton>>) {
    if mouse.just_pressed(bevy::input::mouse::MouseButton::Left) || mouse.just_pressed(bevy::input::mouse::MouseButton::Right) {
        ...
        let (ori, dir) = (cam.translation, cam.rotation * Vec3::NEG_Z);
        let (mut hit, mut prev) = (None, None);
        for i in 0..60 { let p = ori + dir * (i as f32 * 0.1); let (bx,by,bz) = (p.x.floor() as i32, p.y.floor() as i32, p.z.floor() as i32);
            if is_block_solid(&world, bx, by, bz) { hit = Some((bx,by,bz)); break; } prev = Some((bx,by,bz)); }
        if mouse.just_pressed(bevy::input::mouse::MouseButton::Left) { if let Some((bx,by,bz)) = hit { world.modify_block(&mut commands, &mut meshes, bx, by, bz, BlockType::Air); } }
        if mouse.just_pressed(bevy::input::mouse::MouseButton::Right) { if let Some((px,py,pz)) = prev { if !is_block_solid(&world, px, py, pz) { let pl = if py<=0{BlockType::Stone}else{BlockType::Grass}; world.modify_block(&mut commands, &mut meshes, px, py, pz, pl); } } }
    }
}
...
fn load_or_create_atlas_sync(asset_server: &AssetServer, images: &mut Assets<Image>) -> Handle<Image> {
    ...
    if Path::new(&full).exists() { 
        println!("🎨 Chargement HD: {}", full.display()); 
        asset_server.load(path) 
    } else { 
        println!("⚠️ Fallback procédural (Basse Résolution)"); 
        create_block_atlas(images) 
    }
}

// 🎨 Génère un atlas procédural 1024x1024 (4x4 tuiles de 256x256)
fn create_block_atlas(images: &mut Assets<Image>) -> Handle<Image> {
    let size = 1024usize; // ✅ Mise à jour 1024
    let tile = 256usize;  // ✅ Mise à jour 256
    let mut data = vec![0u8; size*size*4];
    
    #[inline] 
    fn set(d: &mut[u8], s:usize, x:usize, y:usize, r:u8, g:u8, b:u8) { 
        if x<s && y<s { let i=(y*s+x)*4; d[i]=r; d[i+1]=g; d[i+2]=b; d[i+3]=255; } 
    }

    // Tile 0: Grass Side (col0, row0)
    for ty in 0..tile { for tx in 0..tile { set(&mut data, size, tx, ty, 101,67,33); }} 
    // Bande verte en haut (0..64px pour 256px de tuile)
    for ty in 0..(tile/4) { for tx in 0..tile { set(&mut data, size, tx, ty, 34,139,34); }}
    
    // Tile 1: Grass Top (col1, row0)
    for ty in 0..tile { for tx in 0..tile { set(&mut data, size, tile+tx, ty, 34,139,34); }}
    
    // Tile 2: Verdure (col2, row0) - Fallback
    for ty in 0..tile { for tx in 0..tile { set(&mut data, size, tile*2+tx, ty, 50,150,50); }}
    
    // Tile 3: Dirt (col3, row0)
    for ty in 0..tile { for tx in 0..tile { 
        let n=(((tx as usize).wrapping_mul(13)^(ty as usize).wrapping_mul(17))&31) as u8; 
        set(&mut data, size, tile*3+tx, ty, 101+n,67+n,33+n); 
    }}
    
    // Tile 4: Stone (col0, row1)
    for ty in 0..tile { for tx in 0..tile { 
        let n=(((tx as usize).wrapping_mul(13)^(ty as usize).wrapping_mul(17))&31) as u8; 
        set(&mut data, size, tx, tile+ty, 120+n,120+n,125+n); 
    }}
    
    // Tile 5: Stone 2 (col1, row1)
    for ty in 0..tile { for tx in 0..tile { set(&mut data, size, tile+tx, tile+ty, 128,128,128); }}
    
    // Tile 6: Sand (col2, row1)
    for ty in 0..tile { for tx in 0..tile { set(&mut data, size, tile*2+tx, tile+ty, 210,180,140); }}
    
    // Tile 7: Gravel (col3, row1)
    for ty in 0..tile { for tx in 0..tile { set(&mut data, size, tile*3+tx, tile+ty, 100,100,100); }}

    // Tile 8: Bedrock (col0, row2)
    for ty in 0..tile { for tx in 0..tile { set(&mut data, size, tx, tile*2+ty, 40,40,45); }}
    
    // Tile 9: Bedrock 2 (col1, row2)
    for ty in 0..tile { for tx in 0..tile { set(&mut data, size, tile+tx, tile*2+ty, 50,50,55); }}
    
    // Tile 10: Wood (col2, row2)
    for ty in 0..tile { for tx in 0..tile { set(&mut data, size, tile*2+tx, tile*2+ty, 90,50,20); }}
    
    // Tile 11: Light Wood (col3, row2)
    for ty in 0..tile { for tx in 0..tile { set(&mut data, size, tile*3+tx, tile*2+ty, 140,100,60); }}
    
    // Tile 12: Leaves (col0, row3)
    for ty in 0..tile { for tx in 0..tile { 
        let n=(((tx as usize).wrapping_mul(13)^(ty as usize).wrapping_mul(17))&31) as u8; 
        set(&mut data, size, tx, tile*3+ty, 20+n,120+n,40+n); 
    }}
    
    // Tile 13: Leaf (col1, row3)
    for ty in 0..tile { for tx in 0..tile { set(&mut data, size, tile+tx, tile*3+ty, 50,180,50); }}
    
    // Tile 14: White (col2, row3)
    for ty in 0..tile { for tx in 0..tile { set(&mut data, size, tile*2+tx, tile*3+ty, 255,255,255); }}
    
    // Tile 15: White (col3, row3)
